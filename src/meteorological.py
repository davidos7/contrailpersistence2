...
def plt_meteorological_contour_averaged_month_and_longitude(variable,variable_name,title,unit="",cmap='Blues',levels=None):
    ...
    plt.show()
def plt_meteorological_contour_averaged_month_and_longitude_app(variable,variable_name,title,axis,fig,unit="",cmap='Blues',levels=None,issr_limit=0.9):
    spaceless_variable_name = variable_name.replace(" ", "_")
    flight_levels = fl
    latitudes = np.arange(-90,91,1)
    ...
    cont = axis.contourf(latitudes, alt, variable_longitude_and_month_averaged, cmap=cmap, levels=levels)
    axis.contour(latitudes, alt, variable_longitude_and_month_averaged, levels=levels,linewidths=0.1,cmap=mpl.cm.gray)
    ...
    c_bar = fig.colorbar(cont, shrink=0.8,ax=axis)
    c_bar.set_label(variable_name+unit)
    c_bar.set_ticks([0,75])
    #plt.title("{}\nAveraged across all Longitudes and Months in {}".format(title, year))
    axis.set_ylabel("Altitude [km]")
    axis.set_xlabel("Latitude [{}]".format(chr(176)))
    #plt.savefig("figures/{}/{}_contour_plot_averaged_across_month_and_longitude.png".format(year, spaceless_variable_name))
...
def plt_meteorological_against_latitude_for_varying_altitude_app(variable,variable_name,title,axis,fig,unit="",alt_plotting = False,plot_only_operating_altitudes=False):
    spaceless_variable_name = variable_name.replace(" ", "_")
    flight_levels = fl
    latitudes = np.arange(-90,91,1)
    mpl.rcParams['axes.prop_cycle'] = plt.cycler("color", plt.cm.gist_gray(np.linspace(0, 1, 2+len(flight_levels[::2]))))
    ...
    for i, fl_it in enumerate(flight_levels[::2]):
        ...
        linestyle = style_reference[i % len(style_reference)]
        if(not plot_only_operating_altitudes):
            if not alt_plotting: axis.plot(latitudes,variable_to_plot,linestyle=linestyle,label="FL{}".format(int(round(fl_it,-1))))
            else: axis.plot(latitudes,variable_to_plot,linestyle=linestyle,label="{} km".format(round((fl2alt(fl_it)/1000),1)))
        else:
            if(i==1):
                variable_to_plot_dashed1 = np.ma.masked_where(latitudes > 35, variable_to_plot)
                latitudes_dashed1 = np.ma.masked_where(latitudes > 35, latitudes)
                ...
                latitudes_solid = np.ma.masked_where((35 > latitudes) & (-35 < latitudes), latitudes)
                variable_to_plot_solid = np.ma.masked_where((35 > latitudes) & (-35 < latitudes), variable_to_plot)
                axis.plot(latitudes_solid, variable_to_plot_solid, "b-",
                         label="{} km [Non-Equatorial]".format(round((fl2alt(fl_it) / 1000), 1)))
                axis.plot(latitudes_dashed, variable_to_plot_dashed, "b:")

            elif(i==3): axis.plot(latitudes, variable_to_plot, "k",
                     label="{} km [Conventional]".format(round((fl2alt(fl_it) / 1000), 1)))
            elif(i==5):
                variable_to_plot_solid1 = np.ma.masked_where(latitudes>25,variable_to_plot)
                latitudes_solid1 = np.ma.masked_where(latitudes>25,latitudes)
    ...
    #plt.title("{}\nAveraged across all Longitudes and Months in {}".format(title,year))
    axis.set_ylabel(variable_name+unit)
    axis.set_xlabel("Latitude [{}]".format(chr(176)))
    axis.set_ylim(0,50)
    axis.set_xlim(-90,90)
    axis.set_xticks([-90,-60,-30,0,30,60,90])
    ...
    if (plot_only_operating_altitudes):
        axis.fill_between((-90,-35),100,0,alpha=0.3,color="tab:blue")
        axis.fill_between((90,35),100,0,alpha=0.3,color="tab:blue")
        axis.fill_between((-25,25),100,0,alpha=0.3,color="tab:red")
        axis.axvline(-25)
        axis.axvline(25)
        axis.axvline(-35)
        axis.axvline(35)
    legend = axis.legend(loc="upper left",title="Altitude",framealpha=1,fontsize=12)
...
def int_to_month(int):
    months = ["Averaged across all Months","January","February","March","April","May","June","July","August","September","October","November","December"]
    month = months[int]
    return month
